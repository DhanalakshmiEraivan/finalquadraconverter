    ...
    useCallback(
      async () => {
        ...
        try {
          ...
          // Compute each user's real conversion count from the conversion table.
          // The previous implementation displayed the profile field (which was
          // never maintained), so every user appeared as 0.
          const conversionCounts: Record<string, number> = {};
          for (const conversion of conversions as any[]) {
            if (conversion.user_id) {
              conversionCounts[conversion.user_id] = (conversionCounts[conversion.user_id] || 0) + 1;
            }
          }
          for (const u of users) {
            u.conversion_count = conversionCounts[u.id] || 0;
          }
                ...
                paymentRequests.map(
                  (payment) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-4">
                        <PaymentScreenshot path={payment.payment_screenshot_path} />
                      </td>
...
function PaymentScreenshot({ path }: { path: string | null }) {
  ...
  const openScreenshot = async () => {
    ...
    setLoading(true);
    try {
      const { data, error } = await supabase.storage.from('payment-screenshots').createSignedUrl(path, 10 * 60);
      if (error) throw error;
      setUrl(data.signedUrl);
    } catch (e) {
      window.alert(e instanceof Error ? e.message : 'Could not open payment screenshot.');
    } finally { setLoading(false); }
  };

  if (!path) return <span className="text-xs font-semibold text-err-500">Missing</span>;
  return (
    <>
      <button type="button" onClick={openScreenshot} disabled={loading} className="inline-flex items-center gap-1.5 rounded-xl bg-brand-50 px-3 py-2 text-xs font-bold text-brand-700 hover:bg-brand-100 disabled:opacity-50"><Eye className="h-3.5 w-3.5" /> {loading ? 'Opening…' : 'View screenshot'}</button>
      {url && <div className="fixed inset-0 z-[120] grid place-items-center bg-ink-950/70 p-4 backdrop-blur-sm" onClick={() => setUrl(null)}>
        <div className="max-h-[90vh] max-w-3xl overflow-auto rounded-3xl bg-white p-3 shadow-float" onClick={(e) => e.stopPropagation()}>
          <img src={url} alt="UPI payment proof" className="max-h-[82vh] w-auto max-w-full rounded-2xl object-contain" />
          <button type="button" onClick={() => setUrl(null)} className="btn-secondary mt-3 w-full">Close</button>
        </div>
      </div>}
    </>
  );
