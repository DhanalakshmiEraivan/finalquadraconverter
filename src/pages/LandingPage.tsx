...
export function LandingPage({ navigate }: Props) {
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="container-page relative pt-16 pb-20 sm:pt-24">
          ...
          <div className="relative mx-auto mt-12 hidden h-10 max-w-4xl sm:block" aria-hidden="true">
            ...
            <div className="qc-orbit qc-orbit-c"><div className="qc-orbit-card"><b className="text-xs text-ink-900">AI OCR</b><span className="mt-1 block text-[10px] text-accent-600">Instant recognition</span></div></div>
          </div>
      ...
      <section className="container-page py-16">
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {categories.map((cat, i) => {
            ...
            const catTools = cat.id === 'signatures' ? [] : tools.filter((t) => t.category === cat.id).slice(0, 4);
            const catCount = cat.id === 'signatures' ? 16 : tools.filter((t) => t.category === cat.id).length;
            return (
              <button
                key={cat.id}
                onClick={() => navigate(cat.id === 'signatures' ? '/signatures' : `/tools/${cat.id}`)}
  ...
  <Icon className="h-5 w-5 text-brand-700" />
                ...
                <div className="mt-4 flex flex-wrap gap-1.5">
                  {cat.id === 'signatures' ? (
                    <span className="rounded-md bg-brand-50 px-2 py-1 text-xs font-semibold text-brand-700 ring-1 ring-brand-100">
                      16 e-signature tools · included in 5 daily credits
                    </span>
                  ) : (
                    <>
                      {catTools.map((t) => (
