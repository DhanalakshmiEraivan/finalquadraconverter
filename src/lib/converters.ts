...
export async function imageToSVG(file: File): Promise<ConvertResult> {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${size.w}" height="${size.h}" viewBox="0 0 ${size.w} ${size.h}"><image width="${size.w}" height="${size.h}" preserveAspectRatio="none" href="${escaped}" xlink:href="${escaped}"/></svg>`;
  return { blob: new Blob([svg], {type:'image/svg+xml'}), filename: `${file.name.replace(/\.[^.]+$/,'')}.svg`, mimeType:'image/svg+xml', preview: svg };
}
...
export async function imagesToPDF(files: File[]): Promise<ConvertResult> {
  ...
  const first = await loadImage(await fileToDataURL(files[0]));
  const pdf = new jsPDF({ orientation: first.naturalWidth >= first.naturalHeight ? 'l' : 'p', unit: 'pt', format: 'a4' });

  for (let i = 0; i < files.length; i += 1) {
    const file = files[i];
    const dataURL = await fileToDataURL(file);
    const img = i === 0 ? first : await loadImage(dataURL);
    if (i > 0) pdf.addPage('a4', img.naturalWidth >= img.naturalHeight ? 'l' : 'p');
  ...
  return { blob: pdf.output('blob'), filename: `${files[0].name.replace(/\.[^.]+$/, '')}${files.length > 1 ? `-${files.length}-images` : ''}.pdf`, mimeType: 'application/pdf' };
}
...
// Advanced QR aliases. Each specialized tool keeps the same hardened QR
// engine while giving users a task-specific workflow and output name.
export async function qrUrl(text: string, size = 512) { return generateQRCode(text, size); }
export async function qrEmail(text: string, size = 512) { return generateQRCode(text.startsWith('mailto:') ? text : `mailto:${text}`, size); }
export async function qrPhone(text: string, size = 512) { return generateQRCode(`tel:${text}`, size); }
export async function qrSms(text: string, size = 512) { return generateQRCode(`sms:${text}`, size); }
export async function qrWifi(text: string, size = 512) { return generateQRCode(text, size); }
export async function qrVcard(text: string, size = 512) { return generateQRCode(text, size); }
export async function qrLocation(text: string, size = 512) { return generateQRCode(text, size); }
export async function qrText(text: string, size = 512) { return generateQRCode(text, size); }
export async function qrEvent(text: string, size = 512) { return generateQRCode(text, size); }
export async function qrApp(text: string, size = 512) { return generateQRCode(text, size); }
export async function qrSocial(text: string, size = 512) { return generateQRCode(text, size); }
export async function qrSvgPro(text: string) { return generateQRCodeSVG(text); }

function normalizeHex(value: string) {
  const raw = value.trim().replace(/^#/, '');
  const six = raw.length === 3 ? raw.split('').map((x) => x + x).join('') : raw;
  if (!/^[0-9a-fA-F]{6}$/.test(six)) throw new Error('Enter a valid HEX color such as #040720.');
...
function textResult(value: string): ConvertResult {
  return { blob: new Blob([value], { type: 'text/plain;charset=utf-8' }), filename: 'color-result.txt', mimeType: 'text/plain' };
}
...
export async function colorHexRgb(input: string) {
  const {hex,r,g,b} = hexRgb(input);
  return textResult(`${hex}\nRGB: rgb(${r}, ${g}, ${b})\nHSL: use the main Color Converter for exact HSL output.`);
}
export async function colorRgbHex(input: string) {
  const parts = input.split(',').map(v => Number(v.trim()));
  if (parts.length !== 3 || parts.some(v => !Number.isFinite(v) || v < 0 || v > 255)) throw new Error('Enter RGB as R,G,B with values from 0 to 255.');
  return textResult(`RGB: ${parts.join(', ')}\nHEX: #${parts.map(v => Math.round(v).toString(16).padStart(2,'0')).join('').toUpperCase()}`);
}
export async function colorComplement(input: string) {
  const {r,g,b} = hexRgb(input);
  return textResult(`Original: #${[r,g,b].map(v=>v.toString(16).padStart(2,'0')).join('').toUpperCase()}\nComplement: #${[255-r,255-g,255-b].map(v=>v.toString(16).padStart(2,'0')).join('').toUpperCase()}`);
}
export async function colorInvert(input: string) { return colorComplement(input); }
export async function colorLighten(input: string) { return textResult(`Base: ${normalizeHex(input)}\nLight 20%: ${shade(input, .2)}\nLight 40%: ${shade(input, .4)}`); }
export async function colorDarken(input: string) { return textResult(`Base: ${normalizeHex(input)}\nDark 20%: ${shade(input, -.2)}\nDark 40%: ${shade(input, -.4)}`); }
export async function colorContrast(input: string) {
  const {r,g,b,hex} = hexRgb(input);
  const lum = (v:number) => { const x=v/255; return x<=.03928?x/12.92:Math.pow((x+.055)/1.055,2.4); };
  const L = .2126*lum(r)+.7152*lum(g)+.0722*lum(b);
  const contrastWhite=(1.05)/(L+.05), contrastBlack=(L+.05)/.05;
  return textResult(`${hex}\nContrast with white: ${contrastWhite.toFixed(2)}:1\nContrast with black: ${contrastBlack.toFixed(2)}:1`);
}
export async function colorPalette(input: string) {
  return textResult([0,.2,.4,.6,.8].map((a,i)=>`${i+1}: ${shade(input, a)}`).join('\n'));
}
export async function colorGradient(input: string) {
  const base=normalizeHex(input); return textResult(`background: linear-gradient(135deg, ${shade(input,-.35)}, ${base}, ${shade(input,.35)});`);
}
export async function colorCss(input: string) {
  const base=normalizeHex(input); return textResult(`:root {\n  --qc-color: ${base};\n  --qc-color-light: ${shade(input,.25)};\n  --qc-color-dark: ${shade(input,-.25)};\n}`);
}
export async function colorAccessibility(input: string) { return colorContrast(input); }
export async function colorRandom(_input: string) {
  const hex='#'+Array.from({length:6},()=>Math.floor(Math.random()*16).toString(16)).join('').toUpperCase();
  return textResult(`Random HEX: ${hex}`);
}
