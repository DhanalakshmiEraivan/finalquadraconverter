...
function App() {
  ...
  const isAuthRoute = route.name === 'auth' || route.name === 'reset-password';
  const isPublicSigningRoute = route.name === 'sign';

  // Redirect to auth if trying to access protected routes without login
  if ((route.name === 'dashboard' || route.name === 'admin' || route.name === 'tool') && !user && !isPublicSigningRoute) {
    return <AuthPage />;
  }
  ...
  if (isPublicSigningRoute) return <PublicSigningPage />;
  ...
  return (
    <div className="app-shell">
      ...
      <main className="flex-1 min-w-0">
        ...
        {route.name === 'reset-password' && <ResetPasswordPage navigate={navigate} />}
        ...
        {route.name === 'features' && <FeaturesPage />}
        {route.name === 'about' && <AboutPage />}

      {route.name === 'contact' && <ContactPage />}
        {route.name === 'security' && <SecurityPage />}
        {route.name === 'signatures' && <SignaturesPage navigate={navigate} />}
